...
public class SwerveModule {
    ...
    private final PIDController turningPidController; //Need to use the built-in PID controller on the Spark-Max's to control the Angle

    private final CANCoder absoluteEncoder; //Connected to the Turning Motor (Is an "Absolute" Encoder because as the robot turns off and power is lost,
    ...
    public SwerveModule(int driveMotorId, int turningMotorId, boolean driveMotorReversed, boolean turningMotorReversed,   //Used to create swerve modules using their Motor ID's
                        int absoluteEncoderId, double absoluteEncoderOffset, boolean absoluteEncoderReversed) {           //and if they are reversed or have an Offset value
        ...
        driveEncoder.setPositionConversionFactor(ModuleConstants.kDriveEncoderRot2Meter); //Converts Driver Motor from Rotations to Meters
        driveEncoder.setVelocityConversionFactor(ModuleConstants.kDriveEncoderRPM2MeterPerSec); //Converts Drive Motor from Revolutions Per Min to Meters Per Second
        turningEncoder.setPositionConversionFactor(ModuleConstants.kTurningEncoderRot2Rad); //Converts Turning Motor Rotations to Radians
        turningEncoder.setVelocityConversionFactor(ModuleConstants.kTurningEncoderRPM2RadPerSec); //Converts Turning Motor Revolutions Per Min to Radians Per Sec

        turningPidController = new PIDController(ModuleConstants.kPTurning, 0, 0); //Initializes the Turning PID Controller

        turningPidController.enableContinuousInput(-Math.PI, Math.PI); //Connects -180 degrees and 180 degrees, essentially telling the computer that We have Circle
    ...
    public void setDesiredState(SwerveModuleState state) {
        ...
        state = SwerveModuleState.optimize(state, getState().angle); //Optimizes so that you will never have to move more than 90 degrees at a time
        // SwerveDriveKinematics.desaturateWheelSpeeds(moduleStates, attainableMaxSpeedMetersPerSecond);
        driveMotor.set(state.speedMetersPerSecond / Constants.DriveConstants.kPhysicalMaxSpeedMetersPerSecond); //Scales Velocity down using Max Speed
